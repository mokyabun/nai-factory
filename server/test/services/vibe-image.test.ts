...
mock.module('../../src/services/novelai', () => ({ encodeVibe: encodeVibeMock }))
...
const { checkVibe, checkVibesForProject, invalidateVibe } =
...
function mockBunFile(exists: boolean, data = new ArrayBuffer(4)) {
    // @ts-expect-error Bun.file is a global we override in tests
    Bun.file = mock(() => ({
