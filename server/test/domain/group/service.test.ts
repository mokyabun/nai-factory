...
const service =
    require('../../../src/domain/group/service') as typeof import('../../../src/domain/group/service')
...
describe('group service', () => {
    ...
    describe('remove', () => {
        ...
        it('calls removeByProject for each child project on deletion', async () => {
            const group = await createGroup()
            await testDb.db.insert(testDb.projects).values({ groupId: group!.id, name: 'p1' }).run()
            await testDb.db.insert(testDb.projects).values({ groupId: group!.id, name: 'p2' }).run()
