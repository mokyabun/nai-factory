...
import { compilePrompts, compileVariables } from '../../src/services/prompt'
import type { CharacterPrompt, Prompt, PromptVariable } from '../../src/types'
...
describe('compilePrompts', () => {
    ...
    it('returns an empty array when given no variable entries', () => {
        const results = compilePrompts(basePrompt, [])
...
describe('compileVariables', () => {
    ...
    it('returns an empty array when there are no variations', () => {
        const result = compileVariables({ a: '1' }, { b: '2' }, [])
